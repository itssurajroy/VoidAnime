...
export default async function StudioPage({
  ...
  const res = await getProducerAnime(studio, currentPage);
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] text-white selection:bg-primary/30 pb-32 relative overflow-hidden">
      ...
      <CatalogHero 
        title={`Studio: ${producerName || studio}`}
        subtitle={`Discover the animation excellence of ${producerName || studio}. High-performance production catalog index.`}
        breadcrumbLabel="Home"
        breadcrumbHref="/home"
        name="shounen" // Use a default high-quality banner
      />

      <div className="container max-w-[1920px] mx-auto px-4 md:px-8 lg:px-16 py-20 relative z-10">
        <div className="space-y-12">
            <div className="flex items-center justify-between">
                <h2 className="text-white/40 text-[12px] font-black uppercase tracking-[0.4em]">Production Results</h2>
