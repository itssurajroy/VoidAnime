...
export default function MangaError({ error, reset }: ErrorProps) {
  useEffect(() => {
  ...
  const errorConfig = isRateLimit
    ...
    ? {
        ...
        description: 'Check your internet connection and try again. The manga servers might also be temporarily down.',
        color: 'text-blue-400',
    ...
    : {
        ...
        description: 'Our servers hit an unexpected error loading this manga. The support team has been notified.',
        ...
        <div className="rounded-[32px] border border-[#2A2A2A] bg-[#1A1A1A]/30 p-6 backdrop-blur-md">
          ...
          <div className="space-y-3">
            {[
              ...
              { name: 'MangaDex Sync', status: 'Online', color: 'text-green-400' },
            ].map(src => (
