...
return (
    ...
    >
      <div className="relative aspect-[2/3] rounded-2xl overflow-hidden bg-card border border-white/5 shadow-lg transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:shadow-primary/20 group-hover:border-primary/30">
        <Link
          href={`/anime/${anime.id}`}
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-20 pointer-events-none">
          <div className="flex items-center gap-1">
            <span className="px-2.5 py-1 bg-gradient-to-r from-primary to-purple-400 text-black text-[9px] font-black uppercase rounded shadow-lg shadow-primary/40">
              {anime.type || 'TV'}
            </span>
            {anime.episodes?.sub && (
        ...
        {anime.rating && (
          <div className="absolute top-3 right-3 z-20 pointer-events-none">
            <div className="flex items-center gap-1 px-2 py-0.5 bg-black/60 backdrop-blur-md border border-white/10 rounded text-white text-[9px] font-black shadow-lg">
        ...
        <div className="absolute bottom-4 left-0 right-0 px-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 z-30">
          <div className="flex items-center gap-2">
            <Link href={`/watch/${anime.id}`} className="flex-1" onClick={(e) => e.stopPropagation()}>
              <Button
                variant="secondary"
                className="w-full h-9 rounded-xl bg-gradient-to-r from-primary to-purple-400 text-black hover:from-purple-400 hover:to-primary font-black uppercase text-[10px] gap-2 shadow-xl shadow-primary/30 hover:shadow-primary/50"
              >
            ...
            <Button
              ...
              className={cn(
                ...
                inWatchlist
                  ? "bg-gradient-to-r from-primary to-purple-400 border-primary text-black shadow-lg shadow-primary/30"
                  : "bg-black/40 border-white/10 text-white hover:bg-primary hover:border-primary hover:text-black hover:shadow-[0_0_15px_rgba(147,51,234,0.4)]"
              )}
            >
        ...
        {/* Hover Highlight with Neon Effect */}
        <div className="absolute inset-0 ring-1 ring-inset ring-white/10 group-hover:ring-primary/50 group-hover:shadow-[0_0_20px_rgba(147,51,234,0.3)] transition-all duration-500 rounded-2xl pointer-events-none z-20" />
      </div>

      {showDetails && (
        <Link href={`/anime/${anime.id}`} className="mt-4 block space-y-1 px-1">
          <h3 className="text-[14px] font-bold text-white group-hover:text-primary transition-colors line-clamp-1 uppercase tracking-tight group-hover:drop-shadow-[0_0_8px_rgba(147,51,234,0.5)]">
            {anime.name}
          </h3>
